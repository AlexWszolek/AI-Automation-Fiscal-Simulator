...
def build_v2_params(ui: dict) -> V2Params:
    ...
    return V2Params(
        ...
        reemployment_haircut=ui["haircut"], lfp_exit_rate=ui["lfp_exit_rate"], ui_weeks=ui["ui_weeks"],
        ...
        state_rate_hike_cap=ui["state_rate_hike_cap"], interest_rate=ui["interest"], ubi_annual=ui["ubi"],
...
def load_backend():
    try:
        ...
        rung1_ok = reabsorption.cache_path(0.30).exists()
...
offshore = sb.slider("Compute pool — offshore leak share", 0.0, 1.0, 0.25, 0.05)
...
price_pt = sb.slider("Price pass-through (deflation)", 0.0, 1.0, 0.3, 0.05)
prod_pt = sb.slider("Productivity pass-through (GDP)", 0.0, 0.1, 0.01, 0.005)

sb.header("④ Government & demand  [H]")
rung = sb.selectbox("Reabsorption rung", [0, 1],
                    format_func=lambda r: "0 — flat haircut" if r == 0 else "1 — service floor",
                    index=1 if rung1_ok else 0)
if rung == 1 and not rung1_ok:
    sb.warning("Rung-1 cache absent — falling back to Rung 0. Build: `python -m fiscal_model.reabsorption`")
    rung = 0
...
ui = dict(mapping=mapping, cog=cog, phys=phys, adoption_path=adoption_path, n_periods=n_periods,
          ...
          offshore_share=offshore, compute_effective_rate=compute_rate, survivor_unbounded=survivor_unbounded,
          ...
          lfp_exit_rate=lfp_exit, ui_weeks=ui_weeks, price_passthrough=price_pt,
          productivity_passthrough=prod_pt, demand=demand, state_resp=state_resp,
          state_cut_share=state_cut_share, state_rate_hike_cap=rate_cap, interest=interest, ubi=ubi,
